...
- Q: How are AWS prerequisites created before the first CI infrastructure plan can run? → A: A one-time, explicitly approved bootstrap may run outside CI from the reviewed feature revision to establish remote state, OIDC roles, certificate validation, initial static-delivery resources, and required one.com DNS records. It MUST NOT publish a wiki release; after bootstrap, recurring infrastructure changes and application releases run only through the approved `main` workflow.
...
- **FR-029**: AWS infrastructure configuration MUST define all recurring cloud resources and policies. A documented, human-assisted, explicitly approved bootstrap MAY run outside CI once to establish account prerequisites, remote state, OIDC roles, certificate validation, the initial static-delivery resources, and one.com DNS records needed before the first CI plan; it MUST NOT publish a wiki release. After bootstrap, recurring infrastructure changes and application releases MUST run through the approved `main` workflow.
...
- **FR-040**: The `deploy` stage MUST contain a job named `app:deploy` that applies the approved infrastructure plan and publishes the exact bundle produced by `app:build` without rebuilding it.
- **FR-041**: `app:deploy` MUST run only for an approved production release from the `main` integration branch, MUST require explicit human authorization before shared AWS mutation, and MUST never run for pull requests.
