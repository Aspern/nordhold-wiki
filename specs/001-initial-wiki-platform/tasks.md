...
- [x] T091 [US6] After T090, rerun the pull-request workflow so the non-resource-mutating `infra:plan` reads the bootstrapped remote state, review and record the result, and require completed human pull-request review plus human integration of the accepted revision into `main` before production release
- [x] T092 [US6] Trigger an approved release of the integrated `main` revision through `.github/workflows/wiki-platform.yml`; require `infra:apply` to apply the fresh CI plan before `app:deploy` publishes the exact bundle, verify all seven exact jobs and trace artifacts, and require `webapp/scripts/verify-release.ts` to confirm the deployed package version, commit SHA, HTTPS root, and direct tower route
- [x] T093 [US6] Append AWS approval, bootstrap, deployment, and smoke-check evidence plus any unresolved production issue to `specs/001-initial-wiki-platform/validation.md`
