...
- [x] T068 [US6] Define GitHub OIDC plus a repository/PR-scoped non-resource-mutating plan role and a protected-production-environment apply role in `infra/bootstrap/iam.tf`; restrict the plan role to required resource `Get`/`List`/`Describe` actions, exact-state-prefix list/get/put access, and exact `.tflock` get/put/delete access, while reserving Terraform resource mutation, S3 publication, and CloudFront invalidation for the apply role
...
- [x] T078 [US6] Add the exact `app:deploy` job restricted to protected approved `main`, production concurrency, matching bundle/plan/revision digest verification, saved-plan apply, scoped S3 synchronization/cache metadata, and targeted CloudFront invalidation without rebuilding in `.github/workflows/wiki-platform.yml`
...
- [ ] T092 [US6] Trigger an approved release of the integrated `main` revision through `.github/workflows/wiki-platform.yml`; require `app:deploy` to apply the fresh CI plan and publish the exact bundle, verify all six exact jobs and trace artifacts, and require `webapp/scripts/verify-release.ts` to confirm the deployed package version, commit SHA, HTTPS root, and direct tower route
