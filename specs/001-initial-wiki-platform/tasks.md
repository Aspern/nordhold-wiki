...
- [ ] T090 [US6] After the pre-bootstrap lifecycle gate and fresh explicit human approval, verify the existing compatible account-wide GitHub OIDC provider, then follow `infra/README.md` outside CI to apply reviewed bootstrap and initial production-infrastructure plans that contain no shared-provider mutation, migrate state to the verified remote keys, add and retain the one.com ACM validation CNAME, confirm certificate issuance, configure protected GitHub OIDC variables/environment, create the empty static-delivery resources, add the one.com `nordhold` CNAME, and verify origin privacy, HTTPS, tags, caches, and state recovery without publishing a wiki bundle or committing account values
